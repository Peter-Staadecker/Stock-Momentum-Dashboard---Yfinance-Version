...
# prior dates until the most recent closing price. [The annualization is done by raising the price change
...
file_name = r"D:\Users\rpsco\Documents\software\python\python stock screeners\yfin momentum v9 - demo.xlsx"
...
resultDF = pd.DataFrame(index = tickersLST, columns = ['Date','Name','Beta', 'Mkt Cap Bllns', "Last Purchase Date",
                                                       'Last Purchase Price','Adj. Closing Price',
...
for ticker in tickersLST:
    ...
    # --------------------------------------------------------------------------------------------------------
    # get closing price, because of yfinance oddity this really means yesterday's closing price or whenever
    # the recent date was when a closing price was last available
    returndate_dt,cl_price = get_price_on_or_before(ticker, date_todayString )
    # move returndate_dt and closing price into resultDF
    resultDF.loc[ticker,'Date'] = returndate_dt
    resultDF.loc[ticker, 'Adj. Closing Price'] = cl_price
...
resultDF["Annualized 1mo % Price Change"] = ((resultDF["Adj. Closing Price"]/resultDF["Price -1mo"])**12-1)*100
resultDF["Annualized 3mo % Price Change"] = ((resultDF["Adj. Closing Price"]/resultDF["Price -3mo"])**4-1)*100
resultDF["Annualized 6mo % Price Change"] = ((resultDF["Adj. Closing Price"]/resultDF["Price -6mo"])**2-1)*100
resultDF["12mo % Price Change"] = ((resultDF["Adj. Closing Price"]/resultDF["Price -12mo"])-1)*100
...
resultDF["Annualized % Price Change Since Last Purchase"] = \
    (resultDF["Adj. Closing Price"]/resultDF["Last Purchase Price"])**(365/resultDF["Days Since Last Purchase"])*\
    100-100
...
with pd.ExcelWriter(file_name, engine='xlsxwriter', engine_kwargs={'options': {'nan_inf_to_errors': True}}) as writer:
    ...
    for dfrow in range(0,len(outputDF)):
        ...
        # 2 decimal places for last purch price and adj. closing price
