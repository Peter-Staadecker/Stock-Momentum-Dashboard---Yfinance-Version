...
# v10 - for the current price find the most recent ticker price rather than yesterday's close
# v11 - yfinance history fromm start date to end date excludes end date. Correct by adding 1 day to end date
...
def get_price_on_or_before(ticker: str, targetdate_str: str):
    ...
    # if the markets on the end date have closed you may get the end data data by specifying start to end+1
    ...
    # for yfinance download start (s) to end (e)  returns data for dates s to e-1 inclusive, not e because
    # end date is exclusive!! so have to add 1 to the target end date
    targetdate_dt = targetdate_dt + relativedelta(days=1)
    targetdate_str = targetdate_dt.strftime("%Y-%m-%d")

    # Try up to 4 days earlier if necessary
    targetdate_minus4dys_dt = targetdate_dt - relativedelta(days=4)
    targetdate_minus4dys_str = targetdate_minus4dys_dt.strftime("%Y-%m-%d")

    historyDF = yf.download(ticker, start = targetdate_minus4dys_str, end = targetdate_str, interval= "1d")
...
file_name = \
    r"D:\Users\rpsco\Documents\software\python\python stock screeners\stock momentum dashboard for github\stock momentum dashboard v11.xlsx"
